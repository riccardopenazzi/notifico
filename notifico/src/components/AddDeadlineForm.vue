            ...
            <v-row>
                <v-list>
                    <v-list-item v-for="(elem, index) in ordereddEmailsToSend" :key="index"
                        ...
                        <template v-slot:append v-if="elem.date.setHours(0, 0, 0, 0) != form.data?.date?.setHours(0, 0, 0, 0)">
                            <v-icon
                                    icon="mdi-delete"
                                    color="red"
...
export default {
    ...
    methods: {
        createUpdateDeadline() {
            ...
            const isCreating = !vars.content.id;
            this.userDeadlineStoreStore.createUpdateDeadline(vars)
                    .then(vars => {
                        if (vars?.success) {
                            let action = isCreating ? 'creata' : 'aggiornata';
                            alertStore.showAlert({
                                title: `Scadenza ${action} con successo`,
                                message: `La scadenza è stata ${action} correttamente`,
                                color: 'success',
                            });
                            useUserDeadlineStore().resetForm();
                            this.$emit('deadlineCorrectRegistration');
                        } else {
        ...
        addDateToEmailsToSendList(userAction=true){
            if (!this.form.data.emailsToSend.some(x => x.date.setHours(0, 0, 0, 0) === this.dateEmailToAdd.setHours(0, 0, 0, 0)) && this.dateEmailToAdd.setHours(0, 0, 0, 0) <= this.form.data.date.setHours(0, 0, 0, 0)) {
        ...
        deleteDateFromEmailsToSendList(dateToRemove){
            this.form.data.emailsToSend = this.form.data.emailsToSend.filter(x => x.date.setHours(0, 0, 0, 0) != dateToRemove.setHours(0, 0, 0, 0));
        },
        ...
        onDateUpdateModel() {
            ...
            this.form.data.emailsToSend = this.form.data.emailsToSend.filter(x => x.date.setHours(0, 0, 0, 0) <= this.form.data.date.setHours(0, 0, 0, 0) && x.userAction);
            this.dateEmailToAdd = this.form.data.date;
    ...
    emits: ['deadlineCorrectRegistration'],
}
</script>
